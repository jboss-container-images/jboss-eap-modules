...
CLI_SCRIPTS_HOME=$JBOSS_HOME/bin/launch/cli
#This is the cli file generated
CLI_SCRIPT_FILE=$CLI_SCRIPTS_HOME/cli-script.cli
#This is the file used to log errors by the CLI execution
CLI_SCRIPT_ERROR_FILE=$CLI_SCRIPTS_HOME/cli-script-error.cli
#The property file used to pass variables to jboss-cli.sh
CLI_SCRIPT_PROPERTY_FILE=$CLI_SCRIPTS_HOME/cli.properties

#Creates the cli files
> ${CLI_SCRIPT_FILE}
> ${CLI_SCRIPT_ERROR_FILE}
#Set the path of the CLI system property error_file, which will be used in the CLI scripts
#to add error messages. After applying the CLI scripts, if there are messages in this file, then
#the server will not start
echo "error_file=${CLI_SCRIPT_ERROR_FILE}" > ${CLI_SCRIPT_PROPERTY_FILE}

chmod -R g+rwX ${CLI_SCRIPTS_HOME}
