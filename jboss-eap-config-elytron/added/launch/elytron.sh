...
function configure_https() {
  ...
  if [ -n "${HTTPS_PASSWORD}" -a -n "${HTTPS_KEYSTORE_DIR}" -a -n "${HTTPS_KEYSTORE}" -a -n "${HTTPS_KEYSTORE_TYPE}" ]; then
    ...
    key_store="<key-store name=\"LocalhostKeyStore\">\n\
    ...
    key_manager="<key-manager name=\"LocalhostKeyManager\" key-store=\"LocalhostKeyStore\">$key_password</key-manager>\n"
    server_ssl_contexts="<server-ssl-context name=\"LocalhostSslContext\" key-manager=\"LocalhostKeyManager\"/>\n"
  ...
  sed -i "s|<!-- ##ELYTRON_KEY_MANAGER## -->|${key_manager}<!-- ##ELYTRON_KEY_MANAGER## -->|" $CONFIG_FILE
  sed -i "s|<!-- ##ELYTRON_SERVER_SSL_CONTEXT## -->|${key_manager}<!-- ##ELYTRON_SERVER_SSL_CONTEXT## -->|" $CONFIG_FILE
  sed -i "s|<!-- ##HTTPS_CONNECTOR## -->|${https_connector}<!-- ##HTTPS_CONNECTOR## -->|" $CONFIG_FILE
