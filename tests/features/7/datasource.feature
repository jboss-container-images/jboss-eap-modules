...
Feature: EAP Openshift datasources
 ...
 Scenario: check mysql datasource with specified DEFAULT_JOB_REPOSITORY and TIMER_SERVICE
  ...
  Scenario: check mysql and postgresql datasource with specified DEFAULT_JOB_REPOSITORY and TIMER_SERVICE set to first ds
    When container is started with env
       | variable                  | value                                                      |
       | DB_SERVICE_PREFIX_MAPPING     | test-postgresql=TEST_POSTGRESQL,test-mysql=TEST_MYSQL  |
       | TEST_MYSQL_DATABASE           | kitchensink-m                                          |
  ...
  Scenario: check mysql and postgresql datasource with specified DEFAULT_JOB_REPOSITORY and TIMER_SERVICE set to second ds
    When container is started with env
       | variable                  | value                                                      |
       | DB_SERVICE_PREFIX_MAPPING     | test-postgresql=TEST_POSTGRESQL,test-mysql=TEST_MYSQL  |
       | TEST_MYSQL_DATABASE           | kitchensink-m                                          |
  ...
  Scenario: check mysql datasource with a TIMER_SERVICE_DATA_STORE which does not match any added datasources
    # Tests settings which override the defaults we checked in the 'check mysql datasource' scenario
    When container is started with env
       | variable                  | value                        |
       | DB_SERVICE_PREFIX_MAPPING | test-mysql=TEST              |
  ...
  Scenario: check postgresql datasource with a TIMER_SERVICE_DATA_STORE which does not match any added datasources
    # Tests settings which override the defaults we checked in the 'check posgresql datasource' scenario
    When container is started with env
       | variable                  | value                            |
       | DB_SERVICE_PREFIX_MAPPING     | test-postgresql=TEST         |
